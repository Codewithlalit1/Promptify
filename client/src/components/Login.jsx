...
const Login = () => {
    ...
    const {setShowLogin,token,setToken,setUser,backendUrl} = useContext(AppContext);
  ...
  return (
    <div className=' fixed absolute top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm 
         ...
         viewport={{once:true}}
          className='relative bg-white p-10 rounded-xl text-slate-500'>
           ...
           {state !=='Login' && <div className='border px-6 py-2 flex items-center gap-2 rounded-full mt-5'>
            ...
            <p className='text-sm text-blue-600 my-4 cursor-pointer'> {state==='Login' ? 'Forget Password?' : ''} </p>
